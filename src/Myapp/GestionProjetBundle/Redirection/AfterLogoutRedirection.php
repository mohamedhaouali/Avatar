...
namespace Myapp\GestionProjetBundle\Redirection;
...
class AfterLogoutRedirection implements LogoutSuccessHandlerInterface
{
    ...
    public function onLogoutSuccess(Request $request)
    {
        ...
        if (in_array('ROLE_ADMIN', $rolesTab, true) || in_array('ROLE_ADMIN', $rolesTab, true) || in_array('ROLE_USER', $rolesTab, true))
            $response = new RedirectResponse($this->router->generate('fos_user_security_login'));
        // otherwise we redirect user to the homepage of website
        else
            $response = new RedirectResponse($this->router->generate('vol_modifier'));
